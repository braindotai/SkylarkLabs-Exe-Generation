...
#     resource_path('crowd.relative.config'),
#     providers = ['CPUExecutionProvider']
# ) # -> to compile exe
session = onnxruntime.InferenceSession(
    resource_path('assets/models/crowd.relative.config'),
    providers = ['CPUExecutionProvider'],
) # -> to test
...
last_frame_idx = None
actual_date = None
tracking_pipeline = TrackingPipeline(iou_threshold = 0.16)
global_frame_idx_to_results = {}
previousCameraTilt, previousFOV = 0.0, 0.0
...
def verify_license():
    ...
    if pc_month > 3 and pc_year >= 2023:
        get_correct_date_time()
...
# out = cv2.VideoWriter('output3.mp4', fourcc, 24, (1280, 720))

@app.post('/inference/')
def inference_view(
    file: str = Form(default = None),
    imgsz: int = Form(default = 736),
    conf: float = Form(default = 0.28),
    iou: float = Form(default = 0.4),
    frame_idx: str = Form(default = '0'),
    sampling_step: str = Form(default = '5'),
    currentCameraTilt: str = Form(default = '0'),
    currentFOV: str = Form(default = '0'),
):
    global last_results, last_ids, last_frame_idx, tracking_pipeline, global_frame_idx_to_results, previousCameraTilt, previousFOV

    frame_idx = int(frame_idx)
    sampling_step = int(sampling_step)
    currentCameraTilt = float(currentCameraTilt)
    currentFOV = float(currentFOV)

    # print('CameraTilt Delts:\t', previousCameraTilt - currentCameraTilt)
    # print('FOV Delts:\t', previousFOV - currentFOV)
    ...
    cv2.putText(
      img = img,
      text = f'CameraTilt Delta: {previousCameraTilt - currentCameraTilt}',
      org = (10, 40),
      fontScale = 1,
      fontFace = 0,
      color = [0, 200, 0],
    ...
    cv2.putText(
      img = img,
      text = f'FOV Delta: {previousFOV - currentFOV}',
      org = (10, 85),
      fontScale = 1,
      fontFace = 0,
      color = [0, 200, 0],
    ...
    previousCameraTilt, previousFOV = currentCameraTilt, currentFOV
    
    deltaCameraTilt = previousCameraTilt - currentCameraTilt
    deltaFOV = previousFOV - currentFOV
    
    print('[INFO] Change in CameraTilt :', deltaCameraTilt)
    print('[INFO] Change in FOV        :', deltaFOV)

    if abs(deltaCameraTilt) > 0.008 or abs(deltaFOV) != 0:
        print('[INFO] Inference done in    :', time.perf_counter() - start_time, '\n')
        return []
    ...
    try:
                ...
                if num_interpolations - 1 > 0 and last_frame_idx != 1:
                    ...
                    for box_idx, (iou_id, current_box) in enumerate(zip(matched_ids, current_boxes_np)):
                        if iou_id in tracking_pipeline.previous_iou_ids:
                            ...
                            # inters = [current_box]
                            # inters.extend([[[-1, -1], [-1, -1]]] * (num_interpolations - 1))

                        inters = np.array(inters).astype(int)
                        if interpolated_previous_boxes.shape[0] == inters.shape[0]:
                            interpolated_previous_boxes[:, box_idx, :, :] = inters
                        else:
                            interpolated_previous_boxes[:, box_idx, :, :] = inters[:len(interpolated_previous_boxes)]
                else:
                    last_frame_idx = frame_idx
                    interpolated_previous_boxes = np.array([current_boxes_np] * num_interpolations)
        ...
        results = global_frame_idx_to_results.pop(frame_idx).tolist() if frame_idx in global_frame_idx_to_results else []
        print('[INFO] Inference done in    :', time.perf_counter() - start_time, '\n')
